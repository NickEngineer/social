...
public class ActivityManagerTest extends AbstractCoreTest {
  ...
  public void testGetActivitiesWithOffsetLimit() throws Exception {
    int initialSize = activityManager.getActivitiesCount(johnIdentity);
    ...
    assertNotNull("johnActivities must not be null", johnActivities);
    assertEquals("johnActivities.size() must return: " + (10 + initialSize), 10 + initialSize, johnActivities.size());
  }
  ...
  public void testGetParentActivity() {
    ...
    assertEquals("parentActivity.getId() must return: " + demoActivity.getId(), demoActivity.getId(), parentActivity.getId());
  ...
  public void testSaveComment() throws Exception {
    ...
    assertEquals("demoComments.get(0).getTitle() must return: " + commentTitle, commentTitle, demoComments.get(0).getTitle());
  ...
  public void testGetCommentsWithListAccess() throws Exception {
    ...
    assertEquals("maryComments.getNumberOfNewer(baseActivity, 10) must return: 5",
                 5,
                 maryComments.getNumberOfNewer(baseActivity));
    assertEquals("maryComments.getNumberOfOlder(baseActivity) must return: 4", 4, maryComments.getNumberOfOlder(baseActivity));
  }
  ...
  public void testSaveLike() throws Exception {
    ...
    assertEquals("demoActivity.getLikeIdentityIds() must return: 0", 0, demoActivity.getLikeIdentityIds().length);
  ...
  public void testGetActivitiesWithListAccess() throws Exception {
    ...
    assertEquals("demoListAccess.getNumberOfNewer(baseActivity) must return: 4",
                 4,
                 demoListAccess.getNumberOfNewer(baseActivity));
    assertEquals("demoListAccess.getNumberOfOlder(baseActivity) must return: 5",
                 5,
  ...
  public void testGetActivitiesOfUserSpacesWithListAccess() throws Exception {
    ...
    assertEquals("demoActivities.getNumberOfNewer(baseActivity) must return: 4",
                 4,
                 demoActivities.getNumberOfNewer(baseActivity));
    assertEquals("demoActivities.getNumberOfOlder(baseActivity) must return: 5",
                 5,
    ...
    assertEquals("demoActivities.getNumberOfNewer(baseActivity) must return 4", 4, demoActivities.getNumberOfNewer(baseActivity));
    assertEquals("demoActivities.getNumberOfOlder(baseActivity) must return 15",
                 15,
                 demoActivities.getNumberOfOlder(baseActivity));
  ...
  public void testGetActivitiesOfConnections() throws Exception {
    int initialDemoSize = activityManager.getActivitiesOfConnections(demoIdentity).size();
    int initialJohnSize = activityManager.getActivitiesOfConnections(johnIdentity).size();
    int initialMarySize = activityManager.getActivitiesOfConnections(maryIdentity).size();
    ...
    assertEquals("demoConnectionActivities.size() must return: " + initialDemoSize,
                 initialDemoSize,
                 demoConnectionActivities.size());
    ...
    assertNotNull("demoConnectionActivities must not be null", demoConnectionActivities);
    assertEquals("demoConnectionActivities.size() must return: " + (10 + initialJohnSize + initialDemoSize),
                 10 + initialJohnSize + initialDemoSize,
                 demoConnectionActivities.size());
    ...
    demoConnectionActivities = activityManager.getActivitiesOfConnections(demoIdentity);
    assertNotNull("demoConnectionActivities must not be null", demoConnectionActivities);
    assertEquals("demoConnectionActivities.size() must return: " + (20 + initialJohnSize + initialDemoSize + initialMarySize),
                 (20 + initialJohnSize + initialDemoSize + initialMarySize),
                 demoConnectionActivities.size());
  ...
  public void testRelationshipActivities() throws Exception {
    RelationshipPublisher relationshipPublisher =
                                                (RelationshipPublisher) getContainer().getComponentInstanceOfType(RelationshipPublisher.class);
    ...
    String johnActivityId = identityStorage.getProfileActivityId(johnIdentity.getProfile(),
                                                                 Profile.AttachedActivityType.RELATIONSHIP);
    ExoSocialActivity johnActivity = activityManager.getActivity(johnActivityId);
    assertNotNull(johnActivity);
    tearDownActivityList.add(johnActivity);
    String rootActivityId = identityStorage.getProfileActivityId(rootIdentity.getProfile(),
                                                                 Profile.AttachedActivityType.RELATIONSHIP);
    ExoSocialActivity rootActivity = activityManager.getActivity(rootActivityId);
    tearDownActivityList.add(rootActivity);
    assertNotNull(rootActivity);
    String demoActivityId = identityStorage.getProfileActivityId(demoIdentity.getProfile(),
                                                                 Profile.AttachedActivityType.RELATIONSHIP);
  ...
  /**
   * Test
   * {@link ActivityManager#recordActivity(Identity, String, String, String)}
   * 
   * @throws Exception
   * @since 1.2.0-Beta3
   */
  ...
  public void testGetActivityFilesIds() throws Exception {
    ...
    ActivityFile activityFile = new ActivityFile("10",
                                                 ...
                                                 "test.png",
                                                 "image/png",
                                                 new ByteArrayInputStream("test.png".getBytes()),
                                                 new Date().getTime());
  ...
  /**
   * Test {@link ActivityManager#getActivityFileById(long)}
   * (ExoSocialActivity))}
   * 
   * @throws Exception
   */
  ...
  /**
   * Test posting activities in case of user is disabled. Activity streams of
   * disabled users should not receive any new activity until the user account
   * is re-enabled. - john and demo post some activities on each user's stream.
   * - make connection between demo and john and check activities count. -
   * disable demo and check the number of activities of john. - demo and john
   * post activities on demo (disabled) and check if posting successfully or
   * not. - re-enable demo and post activities on demo stream to check if
   * posting successfully or not.
   */
  public void testActivitiesOfDisableUsers() throws Exception {
    ...
    // john post activity, demo is in disabling status so activity reference is
    // not created.
    createActivity(johnIdentity);

    // john post activity with mention case, demo is in disabling status so
    // activity reference is not created.
    createActivityHasMention(johnIdentity, demoIdentity);
   ...
   */
  /*
   * public void testAddProviders() { activityManager.addProcessor(new
   * FakeProcessor(10)); activityManager.addProcessor(new FakeProcessor(9));
   * activityManager.addProcessor(new FakeProcessor(8)); ExoSocialActivity
   * activity = new ExoSocialActivityImpl(); activity.setTitle("Hello");
   * activityManager.processActivitiy(activity); //just verify that we run in
   * priority order assertEquals("Hello-8-9-10", activity.getTitle()); } class
   * FakeProcessor extends BaseActivityProcessorPlugin { public
   * FakeProcessor(int priority) { super(null); super.priority = priority; }
   * @Override public void processActivity(ExoSocialActivity activity) {
