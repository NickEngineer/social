...
public class IdentityManagerTest extends AbstractCoreTest {
  ...
  public void testGetIdentityById() {
    ...
    Identity foundIdentity = identityManager.getOrCreateIdentity(OrganizationIdentityProvider.NAME, username, true);
    ...
    // Gets Identity By Node Id
    {
      ...
      assertEquals("gotIdentity.getRemoteId() must return: " + username, username, gotIdentity.getRemoteId());
      // By default, when getIdentity(String nodeId) will have load profile by
      // default (means saved).
      assertNotNull("gotIdentity.getProfile().getId() must not return: null", gotIdentity.getProfile().getId());

      assertNotNull("gotIdentity.getProfile().getProperty(Profile.FIRST_NAME) must not be null",
                    gotIdentity.getProfile().getProperty(Profile.FIRST_NAME));
      assertFalse("gotIdentity.getProfile().getFullName().isEmpty() must be false",
                  gotIdentity.getProfile().getFullName().isEmpty());
  ...
  public void testGetIdentityByIdWithLoadProfile() {
    ...
    final String username = "root";
    Identity tobeSavedIdentity = identityManager.getOrCreateIdentity(OrganizationIdentityProvider.NAME, username, true);
  ...
  public void testDeleteIdentity() {
    ...
    Identity tobeSavedIdentity = identityManager.getOrCreateIdentity(OrganizationIdentityProvider.NAME, username, false);
    ...
    assertNotNull("gotIdentity must not be null because " + username + " is in organizationService", gotIdentity);
  ...
  public void testGetOrCreateIdentity() {
    ...
    // load profile = true
    {
      gotIdentity1 = identityManager.getOrCreateIdentity(OrganizationIdentityProvider.NAME, username1, true);
    ...
    // load profile = false
    {
      gotIdentity2 = identityManager.getOrCreateIdentity(OrganizationIdentityProvider.NAME, username2, false);
      assertNotNull("gotIdentity2.getId() must not be null", gotIdentity2.getId());

      assertNotNull("gotIdentity2.getProfile().getId() must not be null", gotIdentity2.getProfile().getId());
    }

    ActivityManager activityManager = (ActivityManager) getContainer().getComponentInstanceOfType(ActivityManager.class);

    assertEquals("activityManager.getActivities(gotIdentity1).size() must be 0",
                 0,
                 activityManager.getActivities(gotIdentity1).size());
    assertEquals("activityManager.getActivities(gotIdentity2).size() must be 0",
                 0,
  ...
  public void testUpdateProfile() throws Exception {
    ...
    Identity identityUpdated =
                             identityManager.getOrCreateIdentity(rootIdentity.getProviderId(), rootIdentity.getRemoteId(), false);
  ...
  public void testCacheManagement() throws ActivityStorageException {
    ...
    Identity gotRootIdentity = identityManager.getOrCreateIdentity(OrganizationIdentityProvider.NAME, "root", true);
    assertNotNull("gotRootIdentity.getId() must not be null", gotRootIdentity.getId());
    assertEquals("gotRootIdentity.getProfile().getProperty(Profile.FIRST_NAME) must be updated: " + newFirstName,
                 newFirstName,
    ...
    Identity gotJohnIdentity = identityManager.getOrCreateIdentity(OrganizationIdentityProvider.NAME, "john");
